# Step-wise DDPM evaluation script (chained diffusion mode)
import os
import torch
...
def eval_metrics(warped, introp):
...
def main():
    ...
    CHECKPOINT = 'log/liver_ddpm2_pn2_tran2_experiment/2025-06-30_16-56-09/checkpoints/best_model.pth'
    SAVE_VTP = True
    T = 1200
    DDIM_STEPS = 500

    print("📦 加载 liver 数据...")
    dataset = LiverDataset(DATA_ROOT, num_points=1024, preload=False)
    dataset = Subset(dataset, range(5))
    dataloader = DataLoader(dataset, batch_size=1, shuffle=False, drop_last=False)

    model = TransformerDDPMRegNet(d_model=128, npoint=1024, use_pred_disp=True).to(DEVICE)
    ...
    with torch.no_grad():
        for i, batch in enumerate(dataloader):
            ...
            # === DDPM chained sampling ===
            x_t = torch.randn_like(gt_disp)
            ...
            for t_val in reversed(range(T)):
                ...
                disp_cur = x0_pred
                x_t = sqrt_alpha_bar * x0_pred + sqrt_one_minus_alpha_bar * eps_theta  # next x_t

                if t_val % 100 == 0 or t_val == T - 1 or t_val == 0:
                    pred_disp_step = x0_pred * disp_std + disp_mean
                    warped_step = preop + pred_disp_step
                    rmse, maxd, chamfer, pcd = eval_metrics(warped_step, introp)
                    print(f"[Step {t_val:4d}] RMSE: {rmse:.6f}, MAX: {maxd:.6f}, Chamfer: {chamfer:.6f}, PC_dist: {pcd:.6f}")
            ...
            rmse_ddpm, max_ddpm, chamfer_ddpm, pc_ddpm = eval_metrics(warped_ddpm, introp)
            rmse_ddim, max_ddim, chamfer_ddim, pc_ddim = eval_metrics(warped_ddim, introp)

            print(f"— DDPM (T={T}) FINAL")
            print(f"   📏 RMSE: {rmse_ddpm:.6f}, MAX: {max_ddpm:.6f}, Chamfer: {chamfer_ddpm:.6f}, PC_dist: {pc_ddpm:.6f}")
            print(f"— DDIM (steps={DDIM_STEPS})")
            print(f"   📏 RMSE: {rmse_ddim:.6f}, MAX: {max_ddim:.6f}, Chamfer: {chamfer_ddim:.6f}, PC_dist: {pc_ddim:.6f}")

            if SAVE_VTP:
                save_dir = f"./eval_vtp_ddpm2pn2loss3trans2et/{folder}"

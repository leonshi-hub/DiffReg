...
class DDPMDeformer(nn.Module):
    ...
    def forward(self, x_t, t_embed, cond_feat):
        ...
        h = torch.cat([x_t, cond_feat, t_embed], dim=-1)  # [B, N, 3+cond+time]
        return self.net(h)
...
class TransformerDDPMRegNet(nn.Module):
    def __init__(self, d_model=128, npoint=1024, use_pred_disp=False):
        super().__init__()
        self.encoder_pre = PointNetEncoder(channel=3, d_model=d_model, use_disp=False, use_pred_disp=use_pred_disp)
    ...
    def forward(self, preop, introp, disp, t, pred_disp=None, return_noise=False):
        ...
        feat_int = self.encoder_int(y_input, disp=None, pred_disp=None)

        feat_pre, feat_int = feat_pre.permute(2, 0, 1), feat_int.permute(2, 0, 1)  # [N, B, d]
        trans_feat = self.transformer(feat_pre, feat_int).permute(1, 0, 2)         # [B, N, d]

        t_embed = get_timestep_embedding(t, self.d_model).unsqueeze(1).expand(-1, N, -1)  # [B, N, d]

        if return_noise:
            return lambda x_t: self.ddpm(x_t, t_embed, trans_feat)
        else:
            raise NotImplementedError("Use training loop to predict x0 or sampling separately.")
